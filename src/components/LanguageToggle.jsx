...
export default function LanguageToggle({ className = '', size = 'md' }) {
  ...
  /* The navbar and the open mobile sheet render a toggle EACH, so both were
   * mounted at once carrying the identical layoutId. Framer Motion treats a
   * shared layoutId as "the same element moved", so the teal pill tried to fly
   * between the two toggles and flickered on every language change and every
   * time the sheet opened. A per-instance id keeps each pill animating within
   * its own switch. */
  const pillId = useId()
